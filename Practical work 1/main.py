...
class Reader(User):
    ...
    def user_system(self, library, user_manager):
        while True:
            ...
            while True:
                ...
                if choice == 1:
                    self.show_availaible_books(library)
                elif choice == 2:
    ...
    def show_availaible_books(self, library):
    ...
    def borrow_book(self, library, current_user, user_manager):
        ...
        try:
            ...
            if selected_book.title in current_user.borrowed_books:
                print(f"Вы уже взяли книгу '{selected_book.title}'!")
                return

            selected_book.status = "Выдана"
            
            current_user.borrowed_books.append(selected_book.title)
    ...
    def return_book(self, current_user, library, user_manager):
        ...
        for i, book_title in enumerate(current_user.borrowed_books, 1):
        ...
        try:
            ...
            book_title = current_user.borrowed_books[book_num - 1]
            ...
            book_to_return.status = "Доступна"
            
            current_user.borrowed_books.pop(book_num - 1)
...
class Library:
        ...
        try:
            with open("books.txt", "w", encoding="utf-8") as f:
                for book in self.books:
                    line_book = f"{book.title}/{book.author}/{book.status}\n"
                    f.write(line_book)
            return True
    ...
    def load_books(self):
        try:
            ...
            print(f"Загружено {len(self.books)} из файла")
    ...
    def save_users(self):
        try:
            with open('users.txt', 'w', encoding='utf-8') as f: 
                for user in self.users:
                    books_str = ":".join(user.borrowed_books)
                    line = f"{user.name}:{books_str}\n"
                    f.write(line)
            return True
        except Exception as e:
                ...
                if line:
                    ...
                    if parts and parts[0]:  
                        ...
                        if len(parts) > 1 and parts[1]: 
                            borrowed_books = parts[1].split(':')
                            user.borrowed_books = [book.strip() for book in borrowed_books if book.strip()]
                        
                        self.users.append(user)
                
                print(f"Загружено {len(self.users)} пользователей")
        except FileNotFoundError:
            print("users.txt не найден.")
        except Exception as e:
